...
const formSchema = v.object({
  ...
  paidBy: v.pipe(
    ...
    v.check((pb) => props.members.some((m) => m.id === pb.id)),
  ),
  participants: v.pipe(
    v.array(
      v.object({
    ...
    v.minLength(1),
    v.check((arr) => arr.every((p) => props.members.some((m) => m.id === p.id)), 'Invalid member'),
  ),
});
const { r$ } = useRegleSchema(formState, formSchema);
