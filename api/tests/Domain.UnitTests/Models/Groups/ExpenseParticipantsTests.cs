...
internal sealed class ExpenseParticipantsTests
{
    ...
    public void SetAmount_PercentualSplit_SetsValue()
    {
        ...
        expense.SetAmountAndParticipantsWithPercentualSplit(
            100,
            new Dictionary<Guid, int> { { Guid.NewGuid(), 50 }, { Guid.NewGuid(), 50 } }
        );
    ...
    public void SetAmount_ExactAmountSplit_DoesNotMatchSumOfParticipantAmounts_Throws()
    {
        ...
        expense.SetAmountAndParticipantsWithExactSplit(
            1000,
            new Dictionary<Guid, decimal> { { Guid.NewGuid(), 500 }, { Guid.NewGuid(), 500 } }
        );

        const decimal amount = 1100;
        Should.Throw<ArgumentException>(() => expense.Amount = amount).ParamName.ShouldBe("Amount");
    }
    ...
    public void SetAmountAndParticipantsWithEvenSplit_SetsSplitTypeAndCreatesParticipants()
    {
        ...
        expense.Participants.ShouldAllBe(p =>
            !p.PercentualShare.HasValue && !p.ExactShare.HasValue
        );
    ...
    [Arguments(-1)]
    [Arguments(0)]
    public void SetAmountAndParticipantsWithPercentualSplit_NegativeOrZeroAmount_Throws(
        decimal amount
    )
    {
        var expense = ExpenseBuilder.Create();
    ...
    public void SetAmountAndParticipantsWithPercentualSplit_NegativePercentage_Throws()
    {
        ...
        Should
            .Throw<ArgumentException>(() =>
                expense.SetAmountAndParticipantsWithPercentualSplit(
                    100,
                    new Dictionary<Guid, int> { { Guid.NewGuid(), 101 }, { Guid.NewGuid(), -1 } }
                )
            )
            .ParamName.ShouldBe("participants");
    }
    ...
    public static IEnumerable<Func<int[]>> GetValidPercentages() =>
        [() => [33, 33, 34], () => [33, 67], () => [65, 35], () => [65, 35, 0]];

    [Test]
    [MethodDataSource(nameof(GetValidPercentages))]
    public void SetAmountAndParticipantsWithPercentualSplit_SetsSplitTypeAndCreatesParticipants(
        int[] percentages
    )
    {
        var expense = ExpenseBuilder.Create();
        ...
        expense.Amount.ShouldBe(amount);
        expense.Participants.ShouldAllBe(p => !p.ExactShare.HasValue);

        foreach (var (id, percentage) in participants)
        {
            var participant = expense
                .Participants.Where(p => p.MemberId == id)
                .ShouldHaveSingleItem();
            participant.PercentualShare.ShouldBe(percentage);
            expense.GetExpenseAmountForMember(id).ShouldBe((percentage / 100.0m) * amount);
        }
    }
    ...
    public void SetAmountAndParticipantsWithExactSplit_EmptyParticipants_Throws()
    {
        ...
        Should
            .Throw<ArgumentException>(() =>
                expense.SetAmountAndParticipantsWithExactSplit(100, new Dictionary<Guid, decimal>())
            )
            .ParamName.ShouldBe("participants");
    }
    ...
    )
    {
        ...
        foreach (var (id, participantAmount) in participants)
        {
            ...
            expense.GetExpenseAmountForMember(id).ShouldBe(participantAmount);
        }
    }
    ...
    public void SetAmountAndParticipantsWithExactSplit_ClearsExistingPercentualValues()
    {
        ...
        expense.Participants.ShouldAllBe(p => !p.PercentualShare.HasValue);
    }
