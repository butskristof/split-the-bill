...
internal sealed class CreatePaymentTests : ApplicationTestBase
{
    ...
    public async Task ReceivingMemberDoesNotExist_ReturnsNotFoundError()
    {
        ...
        await Application.AddAsync(
            new GroupBuilder()
                .WithId(groupId)
                .WithMembers([Tests.Shared.TestData.TestMembers.Alice.Id])
                .Build()
        );
    ...
    public async Task ValidPaymentRequest_PersistsPayment()
    {
        ...
        group!.Payments
            .ShouldHaveSingleItem()
            .ShouldSatisfyAllConditions(
                p => p.SendingMemberId.ShouldBe(Tests.Shared.TestData.TestMembers.Alice.Id),
                p => p.ReceivingMemberId.ShouldBe(Tests.Shared.TestData.TestMembers.Bob.Id),
