...
public sealed class Expense
{
    ...
    public decimal Amount
    {
        ...
        {
            if (value <= 0)
                throw new ArgumentException("Amount should be a positive value", nameof(Amount));
            if (
                SplitType == ExpenseSplitType.ExactAmount
                && value != _participants.Sum(p => p.ExactShare)
            )
                throw new ArgumentException(
                    "Amount should equal sum of exact split amounts",
                    nameof(Amount)
                );
            field = value;
        }
    ...
    public void SetAmountAndParticipantsWithEvenSplit(
        decimal amount,
    ...
    )
    {
        if (participants.Count == 0)
            throw new ArgumentException(
                "List of participants cannot be empty",
                nameof(participants)
            );
    ...
    public void SetAmountAndParticipantsWithPercentualSplit(
        decimal amount,
        IReadOnlyDictionary<Guid, int> participants
    )
    {
        if (participants.Count == 0)
            throw new ArgumentException(
        ...
        if (participants.Any(p => p.Value < 0))
            throw new ArgumentException(
                "All participant percentages should be at least 0",
                nameof(participants)
            );
        if (participants.Sum(p => p.Value) != 100)
            throw new ArgumentException(
                "Sum of participant percentages should add up to 100",
                nameof(participants)
            );
        ...
        _participants.AddRange(
            participants.Select(pair => new ExpenseParticipant
            {
                MemberId = pair.Key,
                PercentualShare = pair.Value,
            })
        );
    }

    public void SetAmountAndParticipantsWithExactSplit(
        decimal amount,
        IReadOnlyDictionary<Guid, decimal> participants
    )
    {
        if (participants.Count == 0)
            throw new ArgumentException(
        ...
        if (participants.Any(p => p.Value < 0))
            throw new ArgumentException(
                "All participant shares should be at least 0",
                nameof(participants)
            );
        if (participants.Sum(p => p.Value) != amount)
            throw new ArgumentException(
                "Sum of participant shares should add up to amount",
                nameof(participants)
            );
    ...
    public decimal GetExpenseAmountForMember(Guid memberId) =>
        Participants.Any(p => p.MemberId == memberId)
            ? SplitType switch
            {
                ...
                ExpenseSplitType.Percentual => Amount
                    * (
                        Participants.Single(p => p.MemberId == memberId).PercentualShare!
                        ?? throw new ArgumentNullException(
                            nameof(ExpenseParticipant.PercentualShare)
                        )
                    )
                    / 100m,
                ExpenseSplitType.ExactAmount => Participants
                    ...
                    .ExactShare!
                    ?? throw new ArgumentNullException(nameof(ExpenseParticipant.ExactShare)),
