...
public static class UpdatePayment
{
    ...
    internal sealed class Validator : BaseValidator<Request>
    {
        public Validator()
        {
            ...
            RuleFor(r => r.PaymentId).NotNullOrEmptyWithErrorCode();
    ...
    internal sealed class Handler : ICommandHandler<Request, ErrorOr<Updated>>
    {
        ...
        {
            _logger.LogDebug(
                "Updating payment with id {PaymentId} in Group with id {GroupId}",
                request.PaymentId,
                request.GroupId
            );
            ...
            _logger.LogDebug(
                "Fetched group to update payment in from database with related navigation properties"
            );
